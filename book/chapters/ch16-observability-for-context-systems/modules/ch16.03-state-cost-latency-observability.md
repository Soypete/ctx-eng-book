...
Tracing one failed run is necessary but insufficient. Production reliability also depends on aggregate signals: workflows stuck in a state, costs concentrated in retries, latency hidden in a tail, or a new prompt version degrading one customer slice.
...
Count input and output tokens, retrieval and reranking work, tool calls, retries, storage, serving compute, human review, and recovery. Attach them to task class, version cohort, and terminal outcome. Chapter 14’s cost-per-success framing prevents a cheap failed attempt from looking efficient.
...
Record queue time, context assembly, each required dependency, model time to first token, generation, tools, validation, and persistence. Use traces to identify the critical path and metrics to monitor p50, p95, and p99 by task class. Track deadlines, cancellations, retries, fan-out, cache age, and degraded-source outcomes.
