...
Rules belong in software when they express invariants. Judgment can belong to the model when the result remains a proposal and the harness can check its consequences. This is bounded autonomy: freedom inside a boundary that does not depend on the model remembering it.
...
The reliability question is not how autonomous the agent appears. It is whether the surrounding system can explain what happened, constrain what may happen next, and recover safely when the model or infrastructure is wrong. Once those controls are explicit, the next constraint is economic: how much context and orchestration the task can justify.
