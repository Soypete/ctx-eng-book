...
The query plan should declare required slots, optional evidence, eligible sources, filters, freshness, deadlines, candidate budgets, fusion, reranking, and stop conditions. The model may help interpret an ambiguous request, but host policy owns authority and source scope.

## Keep Candidate Stages Observable

Candidate generation optimizes cheap recall. Reranking spends more computation on a smaller set using richer query–document interaction or task features. Final selection satisfies context role and token budgets.
