...
An agent was tasked with processing a batch of 100 records. Each record required a lookup, a transformation, and a write. The agent would loop through each record individually. The cost per record: ~$0.10 (illustrative; actual costs vary by model). Total cost: ~$10.
