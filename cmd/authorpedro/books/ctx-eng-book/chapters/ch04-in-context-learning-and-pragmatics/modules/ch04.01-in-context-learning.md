...
This is the core insight: **more inference by the model means less reliability**. Context engineering flips this — we do the inference work so the model can just extract. [Brown et al., 2020](https://arxiv.org/abs/2005.14165) introduced few-shot learning as in-context learning, but the reliability insight is from context engineering: the more the model must infer, the less reliable the output.
...
The teleprompter pattern (from the [DSPy paper](https://arxiv.org/abs/2310.05914)) is the distinction between:
