...
Agents run for minutes or hours. Infrastructure fails in seconds. Durable execution ensures agent progress survives system failures: crashes, restarts, network partitions, scale-in events. This connects to research on agent memory as a "durable control channel" that can reshape how agents interpret tasks and execute actions [Zhang et al., 2026](https://arxiv.org/abs/2606.06054).
