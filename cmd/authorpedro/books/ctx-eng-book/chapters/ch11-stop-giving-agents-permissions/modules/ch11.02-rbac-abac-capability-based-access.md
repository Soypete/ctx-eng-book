...
- [See research/semantic-contracts.md] — Semantic contracts govern warranted interpretation while pragmatic contracts govern warranted action: they control what the agent can appropriately do—retrieve information for a specific purpose, propose changes but not execute them, request clarification when intent is ambiguous, obtain confirmation before irreversible operations, choose tools that match the user's institutional goal.